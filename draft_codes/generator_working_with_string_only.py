...
#     item_type = options.get('item_type', {'type': 'string'})
...
def generate_array(options):
    ...
    # Handle item_type whether it's a string or a dict
    if isinstance(item_type, dict):
        actual_type = item_type.get('type', 'string')
    else:
        actual_type = item_type
    
    return [generate_field_value(actual_type, item_options) for _ in range(length)]
